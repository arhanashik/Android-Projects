...
public class SeekArc extends View {
	...
	/**
	 * Enable touch outside the SeekArc
	 */
	...
	private float mTouchIgnoreEndRadius;
	private float mTouchIgnoreStartRadius;
	...
	private void init(Context context, AttributeSet attrs, int defStyle) {
		...
		if (attrs != null) {
			...
			mTouchInside = a.getBoolean(R.styleable.SeekArc_touchInside,
					mTouchInside);
			mTouchOutside = a.getBoolean(R.styleable.SeekArc_touchOutside,
					mTouchOutside);
	...
	private boolean ignoreTouch(float xPos, float yPos) {
		...
		if (touchRadius < mTouchIgnoreEndRadius && touchRadius > mTouchIgnoreStartRadius) {
			ignore = true;
		}
		return ignore;
	}
	...
	public void setTouchInSide(boolean isEnabled) {
		...
		if (mTouchInside) {
			mTouchIgnoreEndRadius = (float) mArcRadius / 4;
		} else {
			// Don't use the exact radius makes interaction too tricky
			mTouchIgnoreEndRadius = mArcRadius
					- Math.min(thumbHalfWidth, thumbHalfheight);
		}
		mTouchIgnoreStartRadius = 0;
	}
	...
	public void setTouchOutSide(boolean isEnabled) {
		...
		if (mTouchOutside) {
			mTouchIgnoreStartRadius = (float) mArcRadius * 4;
		} else {
			// Don't use the exact radius makes interaction too tricky
			mTouchIgnoreStartRadius = mArcRadius
					+ Math.min(thumbHalfWidth, thumbHalfheight);
		}

		mTouchIgnoreEndRadius = (float) mArcRadius;
	}
